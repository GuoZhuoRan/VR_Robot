...
import  other_file.avp_stream.streamer_dev  as streamer_dev # type: ignore
import vr_test
...
class RobotControllerUI(QWidget):
    ...
    def create_shoulder_layout(self, shoulder_name, coordinates):
        ...
        for coord in ['x', 'y', 'z']:
            ...
            coord_label = QLabel(f'{coord.upper()}: {coordinates[coord]:.1f}')
            coord_layout.addWidget(coord_label)

            inc_btn = QPushButton('+')
            dec_btn = QPushButton('--')
            inc_btn.clicked.connect(lambda _, c=coord, l=coord_label: self.update_coordinate(coordinates, c, l, 0.1))
            dec_btn.clicked.connect(lambda _, c=coord, l=coord_label: self.update_coordinate(coordinates, c, l, -0.1))
    ...
    def update_coordinate(self, coordinates, coord, label, increment):
    ...
    def initUI(self):        
        ...
        self.left_shoulder = {'x': 0.2, 'y': 0.2, 'z': 0.3}
        self.right_shoulder = {'x': -0.1, 'y': 0.5, 'z': 0.2}
        ...
        #guozi:add left shoulder controller
        grid.addLayout(self.create_shoulder_layout('Left Shoulder', self.left_shoulder), 13, 0)
        grid.addLayout(self.create_shoulder_layout('Right Shoulder', self.right_shoulder), 13, 1)
